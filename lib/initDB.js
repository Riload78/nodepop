...
const initAdverts = async () => {
  try {
    const ask = await confirm('The Adverts database was deleted. Are yor sure? (y/n) ')
    if (!ask) process.exit()
...
const initUsers = async () => {
  try {
    const ask = await confirm('The Users database was deleted. Are yor sure? (y/n) ')
    if (!ask) process.exit()
    await User.deleteMany({}) // Delete all documents from the collection
    console.info('Database is clean')
    await User.insertMany(userData) // Insert data into the database
    console.info(`${userData.length} insert document`)
  } catch (error) {
    console.error(error)
  }
}
